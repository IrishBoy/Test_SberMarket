...
#яркий пример товара без обозначения типа продажи на развес - это Креветки охлажденные тигровые 15/25 , Таиланд
            retailer_price_kilo = retailer_price
            if pack_type == 'Поштучно':
                pack_type = 'Весовой'
        else:
            retailer_price_kilo = 0
        dataFrame = dataFrame.append(pd.Series([offer_id, name, items_amount,
                                                pack_type, vat, shop_price,
                                                retailer_price_kilo,
                                                retailer_price,
